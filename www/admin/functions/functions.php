...
function edit_page($page_id){
    ...
    if(empty($title)){
        // если нет названия
        $_SESSION['edit_page']['res'] = "<div class='error'>Має бути назва сторінки!</div>";
        return false;
    }else{
        ...
        if(mysqli_affected_rows() > 0){
            ...
            $_SESSION['edit_page']['res'] = "<div class='error'>Помилка чи Ви нічого не міняли!</div>";
            return false;
        }
    }
}
...
function add_page(){
    ...
    if(empty($title)){
        // если нет названия
        $_SESSION['add_page']['res'] = "<div class='error'>Має бути назва сторінки!</div>";
        ...
        if(mysqli_affected_rows() > 0){
            ...
            $_SESSION['add_page']['res'] = "<div class='error'>Помилка при додаванні сторінки!</div>";
            return false;
        }
    }
}
...
function add_news(){
    ...
    if(empty($title)){
        // если нет названия
        $_SESSION['add_news']['res'] = "<div class='error'>Має бути назва новини!</div>";
        ...
        if(mysqli_affected_rows() > 0){
            ...
            $_SESSION['add_news']['res'] = "<div class='error'>Помилка при додаванні новини!</div>";
            return false;
        }
    }
}
...
function edit_news($news_id){
    ...
    if(empty($title)){
        // если нет названия
        $_SESSION['edit_news']['res'] = "<div class='error'>Має бути назва новини!</div>";
        return false;
    }else{
        $title = clear_admin($title);
        $keywords = clear_admin($keywords);
        ...
        if(mysqli_affected_rows() > 0){
            ...
            $_SESSION['edit_news']['res'] = "<div class='error'>Помилка чи Ви нічого не міняли!</div>";
            return false;
        }
    }
}
...
function del_news($news_id){
    ...
    if(mysqli_affected_rows() > 0){
        ...
        $_SESSION['answer'] = "<div class='error'>Помилка видалення новини!</div>";
        return false;
    }
}
/* ===Удаление новости=== */
...
function add_link(){
    ...
    if(empty($link_name)){
        // если нет названия
        $_SESSION['add_link']['res'] = "<div class='error'>Має бути назва сторінки!</div>";
        ...
        if(mysqli_affected_rows() > 0){
            ...
            $_SESSION['add_link']['res'] = "<div class='error'>Помилка при добавлені сторінки!</div>";
            return false;
        }            
    }
}
...
function edit_link($link_id){
    ...
    if(empty($link_name)){
        // если нет названия
        $_SESSION['edit_link']['res'] = "<div class='error'>Має бути назва сторінки!</div>";
        return false;
    }else{
        $link_name = clear_admin($link_name);
        $keywords = clear_admin($keywords);
        ...
        if(mysqli_affected_rows() > 0){
            ...
            $_SESSION['edit_link']['res'] = "<div class='error'>Помилка при редагувані сторінки!</div>";
            return false;
        }
    }
}
...
function add_informer(){
    ...
    if(empty($informer_name)){
        $_SESSION['add_informer']['res'] = "<div class='error'>В інформера має бути назва!</div>";
        return false;
    }else{
        ...
        if(mysqli_affected_rows() > 0){
            ...
            $_SESSION['add_informer']['res'] = "<div class='error'>Помилка добавленя інформера!</div>";
            return false;
        }
    }
}
...
function edit_informer($informer_id){
    ...
    if(empty($informer_name)){
        $_SESSION['edit_informer']['res'] = "<div class='error'>У інформера має бути назва</div>";
        return false;
    }else{
        ...
        if(mysqli_affected_rows() > 0){
            ...
            $_SESSION['edit_informer']['res'] = "<div class='error'>Помилка оновлення інформера!</div>";
            return false;
        }
    }
}
...
function add_brand(){
    ...
    if(empty($brand_name)){
        $_SESSION['add_brand']['res'] = "<div class='error'>Ви не вказали назву категорії</div>";
        return false;
    }else{
        ...
        if(mysqli_num_rows($res) > 0){
            ...
            if(mysqli_affected_rows() > 0){
                ...
                $_SESSION['add_brand']['res'] = "<div class='error'>Помилка при додаванні категорії!</div>";
                return false;
            }                        
        }
    }
...
function edit_brand($brand_id){
    ...
    if(empty($brand_name)){
        $_SESSION['edit_brand']['res'] = "<div class='error'>Ви не вказали назву категорії</div>";
        return false;
    }else{
        ...
        if(mysqli_num_rows($res) > 0){
            ...
            if(mysqli_affected_rows() > 0){
                ...
                $_SESSION['edit_brand']['res'] = "<div class='error'>Помилка при редагувані категорії!</div>";
                return false;
            }
        }
    }
...
function edit_product($id){
    ...
    if(empty($name)){
        $_SESSION['edit_product']['res'] = "<div class='error'>У товару має бути назва</div>";
        return false;
    }else{
        ...
        if($_FILES['baseimg']['name']){
            ...
            $error = "";
            if(!in_array($baseimgType, $types)) $error .= "Допустимі розширення - .gif, .jpg, .png <br />";
            if($baseimgSize > SIZE) $error .= "Максимальна вага файлу - 1 Мб";
            if($baseimgError) $error .= "Помилка при загрузці файлу. Можливо, файл дуже великий";
            
            if(!empty($error)) $_SESSION['answer'] = "<div class='error'>Помилка при загрузці картинки товару! <br /> {$error}</div>";
        ...
        /* базовая картинка */
        $_SESSION['answer'] .= "<div class='success'>Товар обновлений</div>";
        return true;
    }
}
/* ===Редактирование товара=== */
...
function add_product(){
    ...
    if(empty($name)){
        $_SESSION['add_product']['res'] = "<div class='error'>У товара має бути назва</div>";
        ...
        if(mysqli_affected_rows() > 0){
            ...
            if($_FILES['baseimg']['name']){
                $baseimgExt = strtolower(preg_replace("#.+\.([a-z]+)$#i", "$1", $_FILES['baseimg']['name'])); // расширение картинки
                $baseimgName = "{$id}.{$baseimgExt}"; // новое имя картинки а так хіба канає
                $baseimgTmpName = $_FILES['bоseimg']['tmp_name']; // временное имя файла
                $baseimgSize = $_FILES['baseimg']['size']; // вес файла 
                $baseimgType = $_FILES['baseimg']['type']; // тип файла 
                $baseimgError = $_FILES['baseimg']['error']; // 0 - OK, иначе - ошибка
                $error = "";
                ...
                if(!empty($error)) $_SESSION['answer'] = "<div class='error'>Помилка при загрузці товару! <br /> {$error}</div>";
                ...
                for($i = 0; $i < count($_FILES['galleryimg']['name']); $i++){
                    ...
                    if($_FILES['galleryimg']['name'][$i]){
                        ...
                        if(!in_array($galleryimgType, $types)){
                            $error .= "Допустимі розширення - .gif, .jpg, .png <br />";
                            $_SESSION['answer'] .= "<div class='error'>Помилка при загружені картинки {$_FILES['galleryimg']['name'][$i]} <br /> {$error}</div>";
                            continue;  
                        }
                        
                        if($galleryimgSize > SIZE){
                            $error .= "Максимальна вага файлу - 1 Мб";
                            $_SESSION['answer'] .= "<div class='error'>Помилка при загружені картинки {$_FILES['galleryimg']['name'][$i]} <br /> {$error}</div>";
                            continue;   
                        }
                        
                        if($galleryimgError){
                            $error .= "Помилка при завантаженні файлу. Можливо, файл занадто великий";
                            $_SESSION['answer'] .= "<div class='error'>Помилка при загружені картинки {$_FILES['galleryimg']['name'][$i]} <br /> {$error}</div>";
                            continue;   
                        }
                        ...
                        if(empty($error)){
                            if(@move_uploaded_file($galleryimgTmpName, "../userfiles/product_img/photos/$galleryimgName")){
                                ...
                                $_SESSION['answer'] .= "<div class='error'>Не вдалося перемістити завантажену картинку. Перевірте права на папки в каталозі /userfiles/product_img/</div>";
                            }
                        }
                    }
                }
            ...
            /* картинки галереи */
            $_SESSION['answer'] = "<div class='success'>Товар доданий</div>";
            return true;
        }else{
            $_SESSION['add_product']['res'] = "<div class='error'>Помилка придодавані товару</div>";
            return false;
        }
    }
}
...
function upload_gallery_img($id){
    ...
    if($_FILES['userfile']['size'] > SIZE){
        $res = array("answer" => "Помилка! Максимальна вага файлу - 1 Мб!");
        exit(json_encode($res));
    }
    
    if($_FILES['userfile']['error']){
        $res = array("answer" => "Помилка! Можливо, файл дуже великий.");
        exit(json_encode($res));
    }
    
    if(!in_array($_FILES['userfile']['type'], $types)){
        $res = array("answer" => "Допустимі розширення - .gif, .jpg, .png");
        exit(json_encode($res));
    }
...
function add_user(){
    ...
    if(empty($password)) $error .= '<li>Не вказаний пароль</li>';
    ...
    if(empty($error)){
        ...
        if($row){
            ...
            if(mysqli_affected_rows() > 0){
                ...
                $_SESSION['add_user']['res'] = "<div class='error'>Помилка!</div>";
        ...
        $_SESSION['add_user']['res'] = "<div class='error'>Не заповнені обов'язкові поля: <ul> $error </ul></div>";
        $_SESSION['add_user']['login'] = $login;
        $_SESSION['add_user']['name'] = $name;
        $_SESSION['add_user']['email'] = $email;
        $_SESSION['add_user']['password'] = $password;
