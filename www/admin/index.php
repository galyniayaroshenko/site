...
// підключення файла функція користувацької частини
require_once '../functions/functions.php';

// підключення файла функція адмістративної частини
require_once 'functions/functions.php';
...
switch($view){
    ...
    case('orders'):
         if(isset($_GET['confirm'])){
            ...
            if(confirm_order($order_id)){
                $_SESSION['answer'] = "<div class='success'>Статус заказу №{$order_id} успішно змінений.</div>";
                }else{
                $_SESSION['answer'] = "<div class='error'>Статус заказу №{$order_id} не вдалося змінити. Можливо, замовлення з таким номером немає або він вже підтверджений.</div>";   
            }
        ...
        if(isset($_GET['del_order'])){
            ...
            if(del_order($order_id)){
                $_SESSION['answer'] = "<div class='success'>Замовлення видалено.</div>";
            }else{
                $_SESSION['answer'] = "<div class='error'>Помилка! Можливо це замовлення було вже видалено.</div>";
            }
    ...
    case('users'):
        // параметры для навигации
        $perpage = 10; // кол-во користувачів на страницу
        if(isset($_GET['page'])){
            $page = (int)$_GET['page'];
            if($page < 1) $page = 1;
        }else{
